...
public class PWRTerminalUI extends LCEGuiInfoContainer {
	private static final ResourceLocation texture = new ResourceLocation("leafia" + ":textures/gui/reactors/leafia_pwr.png");
	...
	public PWRTerminalUI(InventoryPlayer invPlayer,TileEntity terminal,PWRData core) {
		...
		fuels = keyElements.getKey();
		controls = keyElements.getValue();
	...
	public void drawScreen(int mouseX,int mouseY,float partialTicks) {
		...
		LeafiaClientUtil.renderTankInfo(core.tanks[0],this,mouseX,mouseY,guiLeft+5,guiTop+containerOffset+61,16,52);
		LeafiaClientUtil.renderTankInfo(core.tanks[1],this,mouseX,mouseY,guiLeft+23,guiTop+containerOffset+61,16,52);
		LeafiaClientUtil.renderTankInfo(core.tanks[2],this,mouseX,mouseY,guiLeft+23,guiTop+containerOffset+32,16,26);
		if (core.tanks[3].getMaxFill() > 0) {
			LeafiaClientUtil.renderTankInfo(core.tanks[3],this,mouseX,mouseY,guiLeft-22,guiTop+containerOffset+25,16,52);
			LeafiaClientUtil.renderTankInfo(core.tanks[4],this,mouseX,mouseY,guiLeft-5,guiTop+containerOffset-3,10,16);
		}
		LeafiaGls._pop();
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks,int mouseX,int mouseY) {
		...
		drawTexturedModalRect(guiLeft,guiTop+containerOffset,0,0,244,118);
		{
			...
			if ((rodGrab == null) ? isHovering : (rodGrab == -1)) {
				...
				tip = LeafiaClientUtil.statusDecimals("desc.leafia._repeated.reactors.controlrods",core.masterControl * 100,0);
			}
			LeafiaGls.pushMatrix();
		...
		for (int i = 0; iter.hasNext(); i++) {
			...
			LeafiaGls._push(); {
				...
				if ((rodGrab == null) ? isHovering : (rodGrab == i)) {
					...
					rodHoverPosition = MathHelper.clamp(rodRange.ratio(mouseY),0,1);
					hoveringRodName = s;
					tip = LeafiaClientUtil.statusDecimals("desc.leafia._repeated.reactors.controlrods",rodPositions[i] * 100,0);
				}
		...
		core.tanks[0].renderTank(guiLeft+5,guiTop+containerOffset+61+52,zLevel,16,52);
		core.tanks[1].renderTank(guiLeft+23,guiTop+containerOffset+61+52,zLevel,16,52);
		core.tanks[2].renderTank(guiLeft+23,guiTop+containerOffset+32+26,zLevel,16,26);

		LeafiaGls.color(1,1,1,1); // for some reason without this the entire thing turns red lol
		tex.bindTexture(texture);
		if (core.tanks[2].getFill() > 0) {
			if (core.warnTicks >= 3)
		...
		if (core.tanks[3].getMaxFill() > 0) {
			...
			if (isPointInRegion(-22,containerOffset-4,14,18,mouseX,mouseY)) {
				tip = LeafiaClientUtil.statusDecimals("desc.leafia._repeated.reactors.compression",Math.pow(10,core.compression),0);
				compressionHover = true;
			}
			core.tanks[3].renderTank(guiLeft-22,guiTop+containerOffset+25+52,zLevel,16,52);
			core.tanks[4].renderTank(guiLeft-5,guiTop+containerOffset-3+16,zLevel,10,16);
