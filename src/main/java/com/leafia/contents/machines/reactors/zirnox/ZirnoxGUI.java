...
public class ZirnoxGUI extends GuiInfoContainer {
	...
	private static ResourceLocation texture = new ResourceLocation("leafia" + ":textures/gui/reactors/leafia_zirnox.png");
	private static ResourceLocation texture_02 = new ResourceLocation("leafia" + ":textures/gui/reactors/leafia_zirnox_02.png");
	...
	void drawTankInfo(int mouseX, int mouseY, ui element, FluidTankNTM tank) {
		//FFUtils.renderTankInfo(this,mouseX,mouseY,guiLeft+element.x,guiTop+element.y-1,element.width,element.height,tank,type);
		tank.renderTankInfo(this,mouseX,mouseY,guiLeft+element.x,guiTop+element.y,element.width,element.height);
	}
	void drawInfo(int mouseX, int mouseY, ui element, String[] desc) {
		this.drawCustomInfo(mouseX,mouseY,guiLeft+element.x,guiTop+element.y-1,element.width,element.height,desc);
	}
	...
	public void drawScreen(int mouseX, int mouseY, float f) {
		...
		IMixinTileEntityReactorZIRNOX mixin = (IMixinTileEntityReactorZIRNOX)entity;
		...
		String[] text0 = I18nUtil.resolveKeyArray("desc.leafia._repeated."+((mixin.getRods() != 0) ? "reactors.on" : "reactors.off"));
		this.drawCustomInfoStat(mouseX,mouseY,guiLeft+176,guiTop+6-1,18,19,mouseX,mouseY,text0);
		
		if(entity.water.getFill() <= 0) {
		...
		if(entity.carbonDioxide.getFill() < 4000) {
		...
		String[] text4 = LeafiaClientUtil.statusDecimals("desc.leafia._repeated.reactors.compression",(short)Math.pow(10,mixin.getCompression()),(byte)0);
	...
	protected void mouseClicked(int x, int y, int i) throws IOException {
    	...
    	if(guiLeft + 180 <= x && guiLeft + 180 + 10 > x && guiTop + 28 < y && guiTop + 28 + 88 >= y) {
			...
			LeafiaPacket._start(entity)
					.__write(IMixinTileEntityReactorZIRNOX.packetKeys.CONTROL_RODS.key,(byte)rods)
					.__sendToServer();
    	}
    	...
    	if(guiLeft + 162 <= x && guiLeft + 162 + 14 > x && guiTop + 127 < y && guiTop + 127 + 18 >= y) {
			...
			LeafiaPacket._start(entity)
					.__write(IMixinTileEntityReactorZIRNOX.packetKeys.COMPRESSION.key,(byte)((mixin.getCompression()+1)%4))
					.__sendToServer();
    	}
		...
		if (isHovering(x,y,ui.valve)) {
			valveOpen = true;
			LeafiaPacket._start(entity)
					.__write(IMixinTileEntityReactorZIRNOX.packetKeys.OPENVALVE.key,true)
					.__sendToServer();
		}
    }
	...
	protected void mouseReleased(int mouseX, int mouseY, int state) {
		...
		if (valveOpen)
			LeafiaPacket._start(entity)
					.__write(IMixinTileEntityReactorZIRNOX.packetKeys.OPENVALVE.key,false)
					.__sendToServer();
		valveOpen = false;
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		int pix = (int)Math.floor(Math.min((mixin.getHullTemp()-20)/(mixin.getMeltingPoint()-20),1)*88);
		...
		pix = (int)Math.floor((1-mixin.getRods()/100f)*88);
		GL11.glColor4f(col,col,col,1.0F);
		Minecraft.getMinecraft().getTextureManager().bindTexture(texture);
		drawTexturedModalRect(guiLeft+181,guiTop+28,238,172-pix,8,pix);
		pix = (int)Math.floor((1-mixin.getRodsTarget()/100f)*88);
		GL11.glColor4f(col,col,col,0.25F);
		drawTexturedModalRect(guiLeft+181,guiTop+28,238,172-pix,8,pix);
		GL11.glColor4f(1.0F,1.0F,1.0F,1.0F);
		if (mixin.getRods() > 0) // Indicator Light
			drawTexturedModalRect(guiLeft+175,guiTop+10,233,239,23,17);

		// CO2 Gauge
		drawTexturedModalRect(guiLeft+5,guiTop+65,238,
				(int)Math.floor(entity.carbonDioxide.getFill()/(float)entity.carbonDioxide.getMaxFill()*6f+0.5f)*12,
				18,12);

		// Pressure Dial
		if (mixin.getPressure() < mixin.getMaxPressure())
			drawTexturedModalRect(guiLeft+5,guiTop+26,220,18*(int)Math.floor(mixin.getPressure()/mixin.getMaxPressure()*12+0.5),18,18);
		else
			drawTexturedModalRect(guiLeft+5,guiTop+26,202+18*mixin.dialX(),198+18*mixin.dialY(),18,18);
		...
		GL11.glColor4f(col,col,col,1.0F);
		drawTexturedModalRect(guiLeft+11,guiTop+123,0,38+26*mixin.valveLevel(),26,26);
		GL11.glColor4f(1.0F,1.0F,1.0F,1.0F);

		// Compression
		drawTexturedModalRect(guiLeft+162,guiTop+127,mixin.getCompression()%2*14,mixin.getCompression()/2*19,14,19);

		// Control Rods
		int level = (int)Math.min(Math.floor(mixin.getRods()/20f),4)-1;
		if (level >= 0)
		...
		entity.water.renderTank(guiLeft+193, guiTop+79, zLevel, 16, 52);
		//FFUtils.drawLiquid(entity.tanks[1], guiLeft, guiTop, zLevel, 16, 52, 24, 86);
		entity.steam.renderTank(guiLeft+179, guiTop+145, zLevel, 10, 16);
	}
}
