...
import com.custom_hbm.explosion.LCEExplosionNT.LCEExAttrib;
...
import com.leafia.init.proxy.LeafiaClientProxy;
...
public class LCETileEntityCore extends TileEntityMachineBase implements ITickable, LeafiaPacketReceiver {
	public enum Cores {
		ams_core_sing(LeafiaSoundEvents.dfc_vs, (intended,distance) ->
				Math.pow(MathHelper.clamp(1 - (distance - 3) / 15, 0, 1), 1.5)),
		ams_core_wormhole(LeafiaSoundEvents.dfc_tw, (intended,distance) ->
				Math.pow(MathHelper.clamp(1 - (distance - 3) / 40, 0, 1), 2)),
		ams_core_eyeofharmony(LeafiaSoundEvents.dfc_eoh, (intended,distance) ->
	...
	public int heat;
	public int color;
	public FluidTankNTM[] tanks = new FluidTankNTM[2];
	...
	public final List<LCETileEntityCoreReceiver> absorbers = new ArrayList<>();
	...
	public LCETileEntityCore() {
		super(3);
		this.tanks[0] = new FluidTankNTM(Fluids.DEUTERIUM, 128000);
		this.tanks[1] = new FluidTankNTM(Fluids.TRITIUM, 128000);
	}
	...
	LCEAudioWrapper client_sfx = null;
	boolean sfxPlaying = false;
	LCEAudioWrapper meltdownSFX = null;
	LCEAudioWrapper overloadSFX = null;
	LCEAudioWrapper extinguishSFX = null;
	LCEAudioWrapper explosionsSFX = null;
	...
	public void validate() {
		...
		if (AddonBase.proxy instanceof LeafiaClientProxy) {
			meltdownSFX = AddonBase.proxy.getLoopedSound(
							LeafiaSoundEvents.dfc_meltdown,
					...
					.setLooped(false);
			extinguishSFX = AddonBase.proxy.getLoopedSound(
							SoundEvents.BLOCK_FIRE_EXTINGUISH,
							SoundCategory.BLOCKS,pos.getX()+.5f,pos.getY()+.5f,pos.getZ()+.5f,
							1f,0.8f
					).setCustomAttentuation((intended,distance)->Math.pow(MathHelper.clamp(1-(distance-50)/500,0,1),6.66))
					.setLooped(false);
			overloadSFX = AddonBase.proxy.getLoopedSound(
							LeafiaSoundEvents.overload,
							SoundCategory.BLOCKS,pos.getX()+.5f,pos.getY()+.5f,pos.getZ()+.5f,
							1f,1
					).setCustomAttentuation((intended,distance)->Math.pow(MathHelper.clamp(1-(distance-20)/300,0,1),6.66))
					.setLooped(false);
			explosionsSFX = AddonBase.proxy.getLoopedSound(
							LeafiaSoundEvents.longexplosion,
							SoundCategory.BLOCKS,pos.getX()+.5f,pos.getY()+.5f,pos.getZ()+.5f,
							1f,1
					).setCustomAttentuation((intended,distance)->Math.pow(MathHelper.clamp(1-(distance-50)/500,0,1),6.66))
					.setLooped(false);
			...
			if (inventory.getStackInSlot(1).getItem() instanceof ItemAMSCore /*&& tanks[0].getFluid() != null && tanks[1].getFluid() != null*/) {
				if (tagA != null && tagB != null) {
					...
					{ // i wanted to redo everything this sucks ASS
						...
						for (LCETileEntityCoreReceiver absorber : absorbers)
				...
				if (jammerPos != null) {
					...
					if (tick) {
						...
						if (explosionIn <= 15 && !finalPhase) {
							...
							nt.ignoreBlockPoses.add(pos);
							nt.addAttrib(LCEExAttrib.FIRE);
							nt.addAttrib(LCEExAttrib.DFC_FALL);
							nt.explode();
							LeafiaColor col = new LeafiaColor(colorCatalyst);
							DFCBlastParticle blast = new DFCBlastParticle((float)col.red,(float)col.green,(float)col.blue,250);
							blast.emit(new Vec3d(pos).add(0.5,0.5,0.5),new Vec3d(0,1,0),world.provider.getDimension(),200);
	...
	public void shock() {
		...
		for (int i = 0; i < 25; i++) {
			...
			RayTraceResult res = LeafiaUtil.leafiaRayTraceBlocks(world,p0,p1,false,true,false);
	...
	public static class DFCShockPacket implements LeafiaCustomPacketEncoder {
		...
		public Consumer<MessageContext> decode(LeafiaBuf buf) {
			...
			return (context)->{
				if (te == null) return;
				if (!(te instanceof LCETileEntityCore)) throw new LeafiaDevFlaw("TileEntity is not a TileEntityCore");
				LCETileEntityCore core = (LCETileEntityCore)te;
