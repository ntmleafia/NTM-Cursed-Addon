...
public class LCETorexRender extends Render<LCETorex> {
	...
	@Override
	public void doRender(LCETorex cloud,double x,double y,double z,float entityYaw,float partialTicks){
	...
	private void doScreenShake(LCETorex cloud,double x,double y,double z,float amplitude){
		...
		if (!cloud.reachedPlayer && cloud.sound) {
			...
			if (amplitude > 48) {
				SoundEvent evt = LeafiaSoundEvents.nuke;
				if (amplitude > 128) {
					if (dist <= 100 + Math.pow(amplitude,0.95))
						evt = LeafiaSoundEvents.nuke_near;
					else if (dist > 300 + amplitude + Math.pow(amplitude,0.8) * 2)
						evt = LeafiaSoundEvents.nuke_far;
				} else
					evt = LeafiaSoundEvents.nuke_smol;
				...
				LeafiaShakecam._addShake(cloud.getInitialPosition(),new LeafiaShakecam.shakeSimple(8f * (amplitude / 100),Ease.BACK,Direction.I).configure(amplitude * 12F,24f,0.5f,null));
				LeafiaShakecam._addShake(cloud.getInitialPosition(),new LeafiaShakecam.shakeSmooth(15f * (amplitude / 100),Ease.QUAD,Direction.I).configure(amplitude * 5F,12f,1.8f,8f));
				...
				cloud.world.playSound(player,cloud.getInitialPosition(),LeafiaSoundEvents.mukeExplosion,SoundCategory.BLOCKS,15,1);
			}
			Vec3d force = LeafiaUtil.getKnockback(player.getPositionVector().add(0,player.eyeHeight,0),cloud.getPositionVector(),amplitude);
	...
	private void cloudletWrapper(LCETorex cloud,float partialTicks) {
		...
		buf.begin(GL11.GL_QUADS, DefaultVertexFormats.PARTICLE_POSITION_TEX_COLOR_LMAP);
		for(Cloudlet cloudlet : cloudlets) {
			if (cloudlet.type != LeafiaTorex.TorexType.CONDENSATION) {
				Vec3 vec = cloudlet.getInterpPos(partialTicks);
	...
	private void flareWrapper(LCETorex cloud,float partialTicks,float flareDuration) {
	...
	private void flashWrapper(LCETorex cloud,float interp,float flashDuration) {
	...
	@Override
	protected ResourceLocation getEntityTexture(LCETorex entity) {
		return null;
	}
}
