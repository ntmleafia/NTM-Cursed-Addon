 ...
 * Tor                            Ex
 */
// Modified version of NTMEE Torex
public class LCETorex extends Entity implements IConstantRenderer {
	//@SideOnly(Side.CLIENT) Using these on fields are forced crashes on servers??? Fuck off.
	static final Set<LCETorex> waitingTorexes = new HashSet<>();
	...
	public static final DataParameter<Float> SCALE = EntityDataManager.createKey(LCETorex.class, DataSerializers.FLOAT);
	public static final DataParameter<Byte> TYPE = EntityDataManager.createKey(LCETorex.class, DataSerializers.BYTE);
	...
	public LCETorex(World p_i1582_1_) {
		super(p_i1582_1_);
...
*/
	...
	public LCETorex setScale(float scale) {
	...
	public LCETorex setType(int type) {
	...
	public class Cloudlet {
		...
		private void update() {
			...
			Vec3 simPos = Vec3.createVectorHelper(LCETorex.this.initPosX - this.initPosX, 0, LCETorex.this.initPosZ - this.initPosZ);
			double simPosX = LCETorex.this.initPosX + simPos.length();
			double simPosZ = LCETorex.this.initPosZ + 0D;
		...
		private Vec3 getRingMotion(double simPosX, double simPosZ) {
			...
			double roller = LCETorex.this.rollerSize * this.rangeMod * 0.25;
		...
		private Vec3 getConvectionMotion(double simPosX, double simPosZ) {
			...
			Vec3 torusPos = Vec3.createVectorHelper(
					(LCETorex.this.initPosX + torusWidth),
					(LCETorex.this.initPosY + coreHeight),
					LCETorex.this.initPosZ);
		...
		private void updateColor() {
			...
			double exY = LCETorex.this.initPosY + LCETorex.this.coreHeight;
			double exZ = LCETorex.this.initPosZ;
			...
			this.color = LCETorex.this.getInterpColor(col, type);
		}
		...
		public float getAlpha() {
			float alpha = (1F - ((float)age / (float)cloudletLife)) * LCETorex.this.getAlpha();
	...
	public static void spawnTorex(World world,LCETorex torex) {
	...
	public static void statFac(World world, double x, double y, double z, float scale, boolean sound) {
		...
		LCETorex torex = new LCETorex(world).setScale(MathHelper.clamp(scale * 0.01F, 0.25F, 5F));
		...
		public static class Handler implements IMessageHandler<TorexFinishPacket, IMessage> {
			...
			public IMessage onMessage(TorexFinishPacket message, MessageContext ctx) {
				Minecraft.getMinecraft().addScheduledTask(() -> {
					for (LCETorex torex : waitingTorexes) {
